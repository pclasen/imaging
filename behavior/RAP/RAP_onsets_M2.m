function [] = RAP_onsets_M2(sub,trim)
% RAP behavior from .mat file 
% event related design with epochs matching duration
% DELAYS are not modeled - subsumed in cue and image events
...
condition = {'R+';'R-';'Ro';'C6+';'C6-';'C6o';'I6+';'I6-';'I6o'};
...
for i = 1:4 % one for each run
    ...
    for g = 1:length(condition)
        switch g         
            case 1; onsets = [rate.positive repmat([2 1],length(rate.positive),1)];
            case 2; onsets = [rate.negative repmat([2 1],length(rate.negative),1)];
            case 3; onsets = [rate.neutral repmat([2 1],length(rate.neutral),1)];
            case 4; onsets = [cue.positive repmat([6 1],length(cue.positive),1)];
            case 5; onsets = [cue.negative repmat([6 1],length(cue.negative),1)];
            case 6; onsets = [cue.neutral repmat([6 1],length(cue.neutral),1)];
            case 7; onsets = [image.positive repmat([6 1],length(image.positive),1)];
            case 8; onsets = [image.negative repmat([6 1],length(image.negative),1)];
            case 9; onsets = [image.neutral repmat([6 1],length(image.neutral),1)];
        end
