...
foreach line ("`cat $file`")
	...
	if ($1 == MIG) then
		...
		if ($phase == P2) then
			...
			if (-d $featdir/glm/peristimts) then
			else
				mkdir $featdir/glm/peristimts
			endif
			...
			foreach run (R1 R2 R3 R4 R5 R6 R7 R8) # phase 2 has 8 runs
				...
				foreach trialCond (TswSF TswNF)
					...
					if (-f $featdir/glm/peristimts/$run\_$trialCond\_$roi.txt) then
					else
						fslmeants -i $rundir/stats/$trialCond.nii.gz -o $featdir/glm/peristimts/$run\_$trialCond\_$roi.txt -m $rundir/rois/$roi\_ntv.nii.gz
					endif
